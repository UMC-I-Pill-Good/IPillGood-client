...
} from '@/features/ranking/types/rankingFilter';
import type { RankingGender } from '@/features/ranking/types/ranking';
...
const FilterBottomSheet = ({
  ...
  const renderOption = (option: string, isSelected: boolean, onClick: () => void) => (
    <TextButton
      key={option}
      type='button'
      text={option}
        ...
        <div className='thin-scrollbar flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto pb-12'>
          ...
          <div className='flex flex-col gap-6'>
            <fieldset className='flex flex-col gap-1'>
              <legend className='text-base font-medium leading-normal text-black'>연령대</legend>
            ...
            <fieldset className='flex flex-col gap-1'>
              <legend className='text-base font-medium leading-normal text-black'>성별</legend>
              <div className='flex flex-wrap gap-2'>
                {GENDER_OPTIONS.map((option) =>
                  renderOption(option, genderToOption(draftFilters.gender) === option, () =>
            ...
            <fieldset className='flex flex-col gap-1'>
              <legend className='text-base font-medium leading-normal text-black'>건강 고민</legend>
              <div className='flex flex-col gap-2'>
                {HEALTH_CONCERN_ROWS.map((row) => (
                  <div key={row.join('-')} className='flex flex-wrap gap-2'>
                    {row.map((option) =>
                      renderOption(option, draftFilters.healthConcern === option, () =>
                        updateDraftFilters({
